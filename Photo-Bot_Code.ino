//For Initialisation of Photo-Bot, it has to be connected to the photo-bot project via Bluetooth interface using the Blynk App

#include <PID_v1.h>
#include <Pixy2.h>
#include <NewPing.h>
#define RH_ENCODER_A 18  //right motor encoder A interrupt pin
#define RH_ENCODER_B 19 // right motor encoder B interrupt pin
#define LH_ENCODER_A 20 //left motor encoder A interrupt pin
#define LH_ENCODER_B 21 //left motor encoder B interrupt pin
#define leftMotorPWM 3 //left motor PWM Control (B)
#define rightMotorPWM 5 //right motor PWM Control (A)
#define rightMotorIN1 11 //right motor H-Bridge Config
#define rightMotorIN2 12
#define leftMotorIN3 9 // right motor H-Bridge Config
#define leftMotorIN4 10
...
//definitions for ultrasonic sensor system
...
#define pingSpeed 150 // Ping frequency (in milliseconds), max ping speed is 35ms per sensor
#define criticalD 30// critical distance for object detection (robot stops immediately)
#define warningD 100 //warning distance for object detection (robot comes to smooth stop)
#define maxDetectionD 500 //maximum detection range for ultrasonic sensor (5m) 

//Motor encoder variables
volatile long leftCount = 0; //encoder ticks counter used in Interrupt Service Routine
...
//Remote Control (RC) variables
double robotSpeed = 1.0; //Operating Photo-Bot speed= 1.0m/s
int leftPWMSpeed = 0; //Initial speed of 0m/s
int rightPWMSpeed = 0; //Initial speed of 0m/s
char RCMode = 1; //Photo-Bot intialises in the RC Mode
char autoForwardMode = 0; //submode- autoForwardMode is not activated on initialisation
...
//Kalman Filter variables initialisation (PIXY tracking filtering)
...
//PID for movement (RC Auto-Forward Mode)
...
PID myPID2(&rightEncoderSpeed, &PWMRSpeed, &desiredRSpeed, 25.0, 150.0, 4.5, DIRECT);//crucial to define the first 3 things as doubles


//#define BLYNK_PRINT DebugSerial
#include <BlynkSimpleSerialBLE.h>

// Auth Token in the Blynk App for each Blynk Acc can be obtained via the Blynk Application
//char auth[] = "5c5ed91503384efeb34521691400c068"; //Isabelle
char auth[] = "ccd11955dec54b7e99a166b6e517204f"; //HC
...
void setup()
{
  ...
  //Motor control pin initialisation
...
void loop()
{
  ...
  // Tracking mode
  if (RCMode == 2) {
    pixy.ccc.getBlocks(); //obtain block size of tracked target using PixyMon software
    ...
    if (objectTooClose == 0) //ensure no objects detected within critical distance
    {
      if (pixy.ccc.numBlocks && pixy.ccc.blocks[0].m_age > 15) //only register blocks if they appear on PIXY screen for more than 15 frames
      {
        pixytime = millis(); //record the last time that the pixy was able to find the object
        for (char j = 1 ; j <= 3; j++) //do an average over multiple readings
        {
          pixy.ccc.getBlocks();
        ...
        if ( middleObjectDetected == 1) { //object detected in the front of Photo-Bot
          yspeed = 1; //Photo-Bot not able to move forward, motion is limited to only spinning left or right
        }
        else if (leftObjectDetected == 1) { //object detected on left of Photo-Bot
          if ((xspeed < 0) && (yspeed > 0)) { //if Photo-Bot instructed to veer left
            yspeed = 1; //Photo-Bot spins left (not allowed to move forward)
          }
        }
        else if (rightObjectDetected == 1) {
          if ((xspeed > 0) && (yspeed > 0)) { //if Photo-Bot instructed to veer right
            yspeed = 1; //Photo-Bot spins right (not allowed to move forward)
          }
        }
...
{ if (RCMode == 1)
  {
    if (objectTooClose == 0) {
      ...
      // Add a 1 to both so that default is 1,1. If default is 0,0 joystick will not be able to register commands
...
//Evaluation of Y Size Error for PIXY
void YEvaluation()
{
...
//Evaluation of X Positional Error for PIXY
void XEvaluation() {
...
//KALMAN FILTERING of PIXY sensor information
